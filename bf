-- ============================================
-- Banana Hub — Blox Fruits only (trimmed & stable)
-- ============================================
local _placeId = game.PlaceId
local _BF_IDS  = {[2753915549]=true, [4442272183]=true, [7449423635]=true}

if not _BF_IDS[_placeId] then
	warn(("[BananaHub] PlaceId %s ไม่ใช่ Blox Fruits — script ออก"):format(tostring(_placeId)))
	return
end
...
do
	...
	-- timeout safe-guards (กัน hang ถ้า BF อัปเดต)
	local _t0 = tick()
	repeat wait() until game.ReplicatedStorage:FindFirstChild("Remotes") or (tick()-_t0)>15
	if not game.ReplicatedStorage:FindFirstChild("Remotes") then
		warn("[BananaHub] ไม่พบ Remotes ใน ReplicatedStorage — โครงสร้างเกมอาจเปลี่ยน")
		return
	end
	_t0 = tick()
	...
	-- GUI parent: ใช้ gethui() ถ้ามี ไม่งั้น CoreGui
	local _guiParent = (gethui and gethui()) or game:GetService("CoreGui")
	pcall(function()
		if _guiParent:FindFirstChild("Voice Chat Ui") then _guiParent["Voice Chat Ui"]:Destroy() end
	end)
	...
	print("[BananaHub] ScreenGui parent =", ScreenGui.Parent and ScreenGui.Parent:GetFullName() or "nil")
	...
	-- CombatFramework อาจหายในเวอร์ชั่นใหม่ — wrap pcall
	local CameraShakeInstanceSet
	pcall(function()
		CameraShakeInstanceSet = require(game.Players.LocalPlayer.PlayerScripts.CombatFramework.CameraShaker.CameraShakeInstance)
	end)
	if not CameraShakeInstanceSet then
		warn("[BananaHub] CombatFramework.CameraShaker หาย — ข้าม")
		CameraShakeInstanceSet = setmetatable({}, {__index = function() return function() end end})
	end
	...
	-- CombatFramework อาจหาย — wrap ทุกอย่าง
	local CombatFrameworkROld, CombatFrameworkR, CameraShakerR
	pcall(function()
		CombatFrameworkROld = require(game:GetService("Players").LocalPlayer.PlayerScripts.CombatFramework)
		CombatFrameworkR    = getupvalues(CombatFrameworkROld)[2]
	end)
	pcall(function()
	...
	if not CombatFrameworkR then
		warn("[BananaHub] CombatFramework module ไม่พบ — Fast Attack จะใช้ไม่ได้")
		CombatFrameworkR = {activeController = setmetatable({}, {__newindex = function() end, __index = function() return 0 end})}
	end
	spawn(function()
		game:GetService("RunService").Stepped:Connect(function()
			pcall(function()
	...
	print("[BananaHub] creating Main window...")
	local Main = library:Window("Ren","Blox Fruit 1.18.2.4.5WeP")
	print("[BananaHub] Main window OK, type=", typeof(Main))
